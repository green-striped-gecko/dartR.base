...
test_that("gl2hiphop recodes SNP genotypes to hiphop coding (0 hom, 1 other hom, 2 het)", {
  h <- gl2hiphop(testset2.gl, verbose = 0)
  ...
  expect_identical(rownames(h), indNames(testset2.gl))
  expect_identical(colnames(h), locNames(testset2.gl))
...
test_that("gl2hiphop rejects SilicoDArT input", {
  ...
  # warning.
  expect_error(gl2hiphop(testset2.gs, verbose = 0), "SilicoDArT")
})

test_that("gl2hiphop is silent at verbose = 0", {
  out <- capture.output(h <- gl2hiphop(testset2.gl, verbose = 0))
  expect_length(out, 0)
})
