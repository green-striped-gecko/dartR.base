...
test_that("gl2bayescan writes the BayeScan GESTE layout for testset2.gl", {
  od <- file.path(tempdir(), "bsc_base")
  dir.create(od, showWarnings = FALSE)

  ret <- gl2bayescan(testset2.gl, outfile = "bayescan.txt", outpath = od,
                     verbose = 0)
  expect_null(ret)
  ...
  expect_equal(length(grep("^\\[pop\\]", bl)), nPop(testset2.gl))
})

test_that("gl2bayescan allele counts match manual counts for pop 1, locus 1", {
  od <- file.path(tempdir(), "bsc_cnt")
  dir.create(od, showWarnings = FALSE)
  invisible(gl2bayescan(testset2.gl, outfile = "cnt.txt", outpath = od,
                        verbose = 0))
  bl <- readLines(file.path(od, "cnt.txt"))

  p1 <- as.character(unique(pop(testset2.gl)))[1]
  msub <- as.matrix(testset2.gl[pop(testset2.gl) == p1, ])
...
test_that("gl2bayescan is silent at verbose = 0 and returns NULL invisibly", {
  ...
  dir.create(od, showWarnings = FALSE)
  out <- capture.output(
    v <- withVisible(gl2bayescan(testset2.gl[1:5, 1:10], outfile = "sil.txt",
...
test_that("gl2bayescan rejects SilicoDArT data", {
  ...
  expect_error(gl2bayescan(testset2.gs[1:5, 1:10], outfile = "gs.txt",
