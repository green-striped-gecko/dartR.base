...
#'
#' This function filters out loci with a read depth below the lower threshold
#' or above the upper threshold. Loci with a read depth equal to a threshold
#' are retained. Use the companion function \code{\link{gl.report.rdepth}} to
#' choose the thresholds.

#' @details
#' Loci for which the read depth metric is missing (NA) are removed, because
#' they cannot be assessed against the thresholds.
#'
...
#'  [default 1000].
#' @param plot.display If TRUE, histograms of read depth before and after
...
#' @seealso \code{\link{gl.report.rdepth}}
...
gl.filter.rdepth <-  function(x,
    ...
    # Remove SNP loci with rdepth < lower or > upper (NA rdepth cannot be
    # assessed and is removed)
    ...
    index <- !is.na(rdepth) & rdepth >= lower & rdepth <= upper
    ...
    if (datatype == "SilicoDArT") {
        plotvar <- x2@other$loc.metrics$AvgReadDepth
    } else if (datatype == "SNP") {
        plotvar <- x2@other$loc.metrics$rdepth
    }
    max <- max(plotvar, upper, na.rm = TRUE)
    max <- ceiling(max / 10) * 10
    if (datatype == "SNP") {
    ...
    if (verbose >= 3) {
        ...
        if (any(is.na(rdepth))) {
            cat("    No. of loci deleted for missing (NA) read depth:",
                sum(is.na(rdepth)), "\n")
        }
