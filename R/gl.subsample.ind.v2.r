...
#' @examples 
#' gl <- gl.subsample.ind.v2(testset.gl, n=30, by.pop=FALSE, replace=TRUE)
#' gl <- gl.subsample.ind.v2(platypus.gl, n=10, by.pop=TRUE, replace=TRUE)

#' @export 
#' @return Returns the subsampled genlight object

gl.subsample.ind.v2 <- function(x,
                  n = NULL,
                  replace = TRUE,
                  by.pop = TRUE,
                  verbose = NULL) {
  ...
  # Generate a random set of n numbers
    if (replace){
      nums <- sample(1:nInd(x), size = n, replace = TRUE)
    } else {
      nums <- sample(1:nInd(x), size = n, replace = FALSE)
    }
    ...
    for (popn in popNames(x)){
      ...
      # Generate a random set of n numbers
      if (replace){
        nums <- sample(1:nInd(tmp), size = n, replace = TRUE)
      } else {
        nums <- sample(1:nInd(tmp), size = n, replace = FALSE)
      }
      tmp <- tmp[nums,]
      if(popcount == 1){
        x2 <- tmp
