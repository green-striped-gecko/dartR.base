...
#' @param plot.display If TRUE, histograms of tag length are displayed in the plot window
#' [default TRUE].
...
gl.report.taglength <- function(x,
  ...
  # SET WORKING DIRECTORY
    ...
    if (datatype == "SNP") {
        title <- "SNP data - Tag Length"
    } else {
        title <- "Fragment P/A data (SilicoDArT) - Tag Length"
    }
    ...
    retained <- unlist(lapply(quantile_res, function(y) {
        res <- sum(nchar.tags >= y, na.rm = TRUE)
    }))
