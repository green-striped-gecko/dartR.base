...
#'
#' The pca object must have been generated from the same genlight object as
#' is passed to this function; the function stops if the number of loadings
#' does not match the number of polymorphic loci.
#' 
...
#' 
#' @export
#' @return The filtered genlight object, returned invisibly.
#'
gl.filter.factorloadings <- function(x,
  ...
  if (!inherits(pca, "glPca")) {
    stop(error("To filter on factor loadings, a glPca object generated by
               gl.pcoa is required\n"))
  } else {
  ...
  if (missing(threshold)) {
    stop(error(
      "Fatal Error: threshold is required; use gl.report.factorloadings to
      choose a value\n"
    ))
  }
  ...
  # this when undertaking the ordination
  input.history <- x@other$history
  x <- gl.filter.monomorphs(x,verbose=0)

  # The pca must have been generated from this genlight object: the loadings
  # are paired to loci by position (glPca loadings carry no locus names)
  if (nLoc(x) != nrow(pca$loadings)) {
    stop(error(
      "Fatal Error: the pca object holds", nrow(pca$loadings),
      "loadings but the genlight object has", nLoc(x),
      "polymorphic loci; the pca must be generated from the same genlight
      object passed to this function\n"
    ))
  }

  # Pull the factor loadings into a dataframe
  df <- data.frame(locus = locNames(x), loading = pca$loadings[, axis])
  if(retain){
    tmp <- df[abs(df$loading) >= threshold,]
    x2 <- gl.keep.loc(x, loc.list = tmp$locus, verbose = 0)
  ...
  # ADD TO HISTORY (a single entry for this function; the internal
  # monomorph-removal and keep.loc calls are implementation detail)
  x2@other$history <- input.history
  nh <- length(x2@other$history)
  x2@other$history[[nh + 1]] <- match.call()
  
  # Prepare the plots
  # get title for plots
  xlabel1 <- paste("Distribution of factor loadings for axis",axis,"(pre-filter)")
  xlabel2 <- paste("Distribution of factor loadings for axis",axis,"(post-filter)")

  # Calculate minimum and maximum graph cutoffs for the loadings
  min <- min(df$loading)
  max <- max(df$loading)
  loading <- NULL
  p1 <-
