...
gl.dist.ind <- function(x,
  ...
  if (datatype == "SNP") {
    ...
    if (method == "simple") {
      dd <- dd <- utils.dist.ind.snp(x, method = 'simple', verbose = verbose)
      if (verbose >= 2) {
        cat(report("  Calculating simple matching distance\n"))
      }
    }
    # Calculate absolute Manhattan distance
    if (method == "manhattan") {
      dd <- dd <- utils.dist.ind.snp(x, method = 'manhattan', verbose = verbose)
      if (verbose >= 2) {
        cat(report("  Calculating Manhattan distance\n"))
      }
    }
    
    # Calculate absolute Czekanowski distance
    if (method == "czekanowski") {
      dd <- dd <- utils.dist.ind.snp(x, method = 'czekanowski', verbose = verbose)
      if (verbose >= 2) {
        cat(report("  Calculating Czekanowski distance\n"))
      }
    }
  ...
  if (datatype == "SilicoDArT") {
    ...
    if (method == "sorensen") {
      if (verbose >= 2) {
        cat(report("  Calculating the Sorensen Distance\n"))
      }
    }
    
    mat <- utils.dist.binary(
      x,
      method = method,
      swap = swap,
      #type="matrix",
