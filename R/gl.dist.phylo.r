...
#'
#' min.tag.length : Sequence tags can vary considerably in length, which
...
#'       which parameter must be given by the user. By default, no gamma
#'       correction is applied. Another assumption is that the base frequencies
#'       are balanced and thus equal to 0.25.
#'      \item
...
#'       parameter must be given by the user. By default, no gamma correction
#'       is applied.
#'      \item
...
#'       variation in substitution rates is possible.
#'      \item
...
#'      \item
#'      pairwise.missing: If TRUE, then missing values in the sequence (NNNs)
#'      will be accommodated in the calculations pair of taxa at a time;
#'      otherwise, the deletion of data at positions in the sequence will be
#'      global (deleted if any missing data at the position in any individual).
#' }
