...
#' v3.7.2 onward. For the (limited) testing I have done with migrate-n and SNP 
#' data, migrate-n seems to be able to handle up to 20k SNPs (v3.7.2) and 3-5k 
#' SNPs in v4 and later. That seems to depend on some memory issues that Peter
#'  Beerli may be able to resolve (or may have resolved by the time you use this
#'   function). To workaround this problem (or simply because you want a faster
#'    analysis), there is the option of subsetting the SNPs to a set number 
#'    \code{n}. If \code{n=NULL}, the genlight object is processed as it. 
#' 
#' This function needs PLINK installed, which is used for intermediate data 
#' manipulation.
#' 
...
gl2migraten <- function(x, 
  ...
  gl2plink(x, plink.bin.path = plink.path, bed_files = FALSE, outfile = "plink.out", 
           outpath = tmp.out)
  ...
  Chr <- snp.name <- gen.pos <- phys.pos <- NA
