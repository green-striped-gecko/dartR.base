...
#' from 20 to 69 base pairs. This function filters out loci whose sequence tag
#' length lies below the lower threshold or above the upper threshold. Loci
#' with a tag length equal to a threshold are retained. Use the companion
#' function \code{\link{gl.report.taglength}} to choose the thresholds.

#' @details
#' Loci for which the TrimmedSequence entry is missing (NA) are removed,
#' because their tag length cannot be assessed against the thresholds.
...
#' @seealso \code{\link{gl.report.taglength}}
#' @export
...
gl.filter.taglength <- function(x,
    ...
    if (lower < 0 | lower > 250) {
        if (verbose >= 1) {
            cat(
                warn(
                    "  Warning: Parameter 'lower' must be an integer between 0 and
                250 , set to 20\n"
                )
            )
        }
    ...
    # Remove loci with tag length outside [lower, upper] (NA tag length
    # cannot be assessed and is removed)
    if (verbose > 1) {
        cat(report(
            "  Removing loci with tag length <",
            lower,
            "or >",
            upper,
            "\n"
    ...
    if (verbose > 2) {
        ...
        if (any(is.na(nchar.tags))) {
            cat(report("  No. of loci deleted for missing (NA) tag length:",
                       sum(is.na(nchar.tags)), "\n"))
        }
    }
