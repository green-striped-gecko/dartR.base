...
#' @param nsim Maximum number of iterations for the fixed-point estimation of
#'   the mean (lambda) of the Poisson distribution [default 1000].
...
#'   sequence tags)
#'   \item Lambda Mean of the Poisson distribution of number of SNPs in the
#'   sequence tags
#' }
    ...
    if (is.na(table(duplicated(b))[2])) {
      ...
      if (verbose >= 1) {
        cat("   Number of sequence tags with secondaries:",
            n.tags.secondaries,
            "\n")
        cat("   Number of secondary SNP loci that would be removed on filtering:",
            n.SNPs.secondaries,
            "\n")
        ...
        cat("   Total Number of invariant sites (including invariant sequence tags):",
            n.invariant,
            "\n")
      }
      if (verbose >= 3) {
