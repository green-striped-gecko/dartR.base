...
#'  repeatability against possible thresholds that might subsequently be
...
gl.report.reproducibility <- function(x,
    ...
    p1 <-
        ...
        coord_flip() +
        plot.theme +
        ylim(c(min(repeatability, na.rm = TRUE), 1)) +
        ylab(" ") +
    ...
    p2 <-
        ...
        coord_cartesian(xlim = c(min(repeatability, na.rm = TRUE), 1)) +
        xlab("Repeatability") +
        ylab("Count") +
        plot.theme
    ...
    retained <- unlist(lapply(quantile_res, function(y) {
        res <- sum(repeatability >= y, na.rm = TRUE)
    }))
