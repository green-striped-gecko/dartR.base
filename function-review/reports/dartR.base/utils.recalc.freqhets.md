# Review: utils.recalc.freqhets (dartR.base)

## Provenance

- Model: claude-fable-5 (Claude Code); Skill: dartr-function-review
  v2.0.0; Base: dev at ddaed27; Branch: review-utils.recalc.freqhets. Reviewed as part
...
- **Standards: FAIL** — the monomorphs check `flag == FALSE` is
  ...
  nonexistent functions; the details text says "homozygote reference" where it means heterozygotes.
- **Spec: FAIL** — SilicoDArT input runs through the diploid 0/1/2
  arithmetic silently (presences counted as heterozygotes: FreqHets mean 0.343 on testset.gs); the metric itself is verified exact
...
seealso corrected; the copy-pasted "homozygote reference" details line fixed; creation-message gate standardised.

## Coverage

test-utils.recalc.freqhets.R — 5 assertions (recomputation anchor, flags-crash
...
and SilicoDArT. PR #303.

```json
{"function": "utils.recalc.freqhets", "package": "dartR.base", "family_mode": "analysis",
 "commit": "ddaed27", "skill_version": "2.0.0",
 "verdict_standards": "FAIL", "verdict_spec": "FAIL",
 "findings": [{"id": "B2", "severity": "MEDIUM", "rules": ["spec"], "loc": "R/utils.recalc.freqhets.r monomorphs check", "status": "applied"},
  {"id": "B3", "severity": "MEDIUM", "rules": ["spec", "DAT"], "loc": "R/utils.recalc.freqhets.r datatype", "status": "applied"},
  {"id": "B1", "severity": "LOW", "rules": ["DOC"], "loc": "R/utils.recalc.freqhets.r visibility", "status": "applied"},
  {"id": "B4", "severity": "LOW", "rules": ["DOC"], "loc": "R/utils.recalc.freqhets.r docs", "status": "applied"}],
 "datasets": ["testset.gl", "testset.gs"],
 "baseline_test": "tests/testthat/test-utils.recalc.freqhets.R",
 "pr": 303}
```
